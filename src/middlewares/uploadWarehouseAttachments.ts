// tptech-backend/src/middlewares/uploadWarehouseAttachments.ts
...
const UPLOAD_DIR = path.join(process.cwd(), "uploads", "warehouses", "attachments");
...
function buildName(req: any, file: Express.Multer.File) {
  const warehouseId = String(req?.params?.id || "warehouse");
  const ext = path.extname(String(file.originalname || "")).toLowerCase();
  const safeExt = ext.length <= 12 ? ext : "";
  return `warehouse_att_${warehouseId}_${Date.now()}_${crypto.randomBytes(6).toString("hex")}${safeExt}`;
}
...
async function uploadToR2(req: any, res: any, next: any) {
  try {
    ...
    const key = `warehouses/attachments/${filename}`;
    ...
    f.filename = filename;
    (f as any)._tpFolder = "warehouses/attachments";
...
export const uploadWarehouseAttachmentMiddleware = [
