// tptech-backend/src/middlewares/uploadEntityAttachments.ts
...
const UPLOAD_DIR = path.join(process.cwd(), "uploads", "entities", "attachments");
...
function buildName(req: any, file: Express.Multer.File) {
  ...
  return `entity_att_${entityId}_${Date.now()}_${crypto.randomBytes(6).toString("hex")}${safeExt}`;
}
...
async function uploadToR2(req: any, res: any, next: any) {
  try {
    ...
    const key = `entities/attachments/${filename}`;
    ...
    f.filename = filename;
    (f as any)._tpFolder = "entities/attachments";
...
export const uploadEntityAttachmentMiddleware = [
