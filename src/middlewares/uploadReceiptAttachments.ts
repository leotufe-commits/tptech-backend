// tptech-backend/src/middlewares/uploadReceiptAttachments.ts
...
const UPLOAD_DIR = path.join(process.cwd(), "uploads", "receipts", "attachments");
...
function buildName(req: any, file: Express.Multer.File) {
  const receiptId = String(req?.params?.id || "receipt");
  const ext = path.extname(String(file.originalname || "")).toLowerCase();
  const safeExt = ext.length <= 12 ? ext : "";
  return `receipt_att_${receiptId}_${Date.now()}_${crypto.randomBytes(6).toString("hex")}${safeExt}`;
}
...
async function uploadToR2(req: any, res: any, next: any) {
  try {
    ...
    const key = `receipts/attachments/${filename}`;
    ...
    f.filename = filename;
    (f as any)._tpFolder = "receipts/attachments";
...
export const uploadReceiptAttachmentMiddleware = [
