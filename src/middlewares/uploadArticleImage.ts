// tptech-backend/src/middlewares/uploadArticleImage.ts
...
const UPLOAD_DIR = path.join(process.cwd(), "uploads", "articles", "images");
...
function randomName(originalname: string, articleId: string) {
  const ext = extFromOriginal(originalname);
  return `article_img_${articleId}_${Date.now()}_${crypto.randomBytes(6).toString("hex")}${ext}`;
}
...
const diskStorage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    const articleId = String(req?.params?.id || "article");
    cb(null, randomName(file.originalname, articleId));
  },
});
...
async function uploadToR2(req: any, res: any, next: any) {
  try {
    ...
    const articleId = String(req?.params?.id || "article");
    const filename = randomName(f.originalname, articleId);
    const key = `articles/images/${filename}`;
    ...
    f.filename = filename;
    (f as any)._tpFolder = "articles/images";
...
export const uploadArticleImageMiddleware = [
