// tptech-backend/src/modules/auth/auth.pin.controller.ts
...
import { signToken, setAuthCookie, fetchUserForAuthById } from "./auth.controller.js";
...
const isLocked = (lockedUntil: Date | null | undefined) => !!lockedUntil && lockedUntil.getTime() > Date.now();
...
export async function setMyPin(req: Request, res: Response) {
  ...
  auditLog(req, { action: "auth.pin_set", success: true, userId: updated.id, tenantId: updated.jewelryId });
...
export async function disableMyPin(req: Request, res: Response) {
  ...
  if (!meUser.quickPinEnabled || !meUser.quickPinHash) return res.status(400).json({ message: "El PIN no está habilitado." });
  if (isLocked(meUser.quickPinLockedUntil as any)) return res.status(429).json(lockPayload(meUser.quickPinLockedUntil as any));
  ...
  auditLog(req, { action: "auth.pin_disable", success: true, userId: updated.id, tenantId: updated.jewelryId });
  return res.json({ ok: true });
}
...
export async function unlockWithPin(req: Request, res: Response) {
  ...
  if (!meUser.quickPinEnabled || !meUser.quickPinHash) return res.status(400).json({ message: "Este usuario no tiene PIN configurado." });
  if (isLocked(meUser.quickPinLockedUntil as any)) return res.status(429).json(lockPayload(meUser.quickPinLockedUntil as any));
  ...
  auditLog(req, { action: "auth.pin_unlock", success: true, userId: meUser.id, tenantId: meUser.jewelryId });
  return res.json({ ok: true });
}
...
export async function quickUsers(req: Request, res: Response) {
  ...
  return res.json({
    ...
    users: users.map((u: any) => {
      const roles = (u.roles ?? [])
        .map((ur: any) => ({ id: ur.roleId, name: ur.role?.name, isSystem: ur.role?.isSystem ?? false }))
...
export async function switchUserWithPin(req: Request, res: Response) {
  ...
  const requireOnUserSwitch =
    typeof (j as any)?.pinLockRequireOnUserSwitch === "boolean" ? Boolean((j as any)?.pinLockRequireOnUserSwitch) : true;
  ...
  if (requireOnUserSwitch && !isValidPin(pin)) return res.status(400).json({ message: "PIN inválido." });
...
export async function setPinLockSettingsForJewelry(req: Request, res: Response) {
  ...
  if (typeof body.pinLockRequireOnUserSwitch === "boolean") data.pinLockRequireOnUserSwitch = body.pinLockRequireOnUserSwitch;
  ...
  auditLog(req, { action: "company.security.pin_lock_set", success: true, userId, tenantId, meta: data });
